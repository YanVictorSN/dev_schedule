...
let CompaniesService = CompaniesService_1 = class CompaniesService {
    ...
    async create(createCompanyDto) {
        try {
            ...
            if (error) {
                this.logger.error('Erro ao cadrastar uma nova empresa no Superbase', error);
                throw error;
            }
            return data;
        }
    ...
    async findAll() {
        const { data, error } = await superbase_config_1.default.from('company').select();
        if (error) {
            throw error;
        }
        return data;
    ...
    async remove(id) {
        try {
            const { error } = await superbase_config_1.default.from('company').delete().eq('id', id);
            if (error) {
